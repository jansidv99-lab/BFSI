...
def test_cache_miss_returns_none():
    assert cache_get("chat", ["what is nifty"]) is None


def test_set_then_get_returns_value():
    cache_set("chat", ["what is nifty"], "result_json", 3600)
    assert cache_get("chat", ["what is nifty"]) == "result_json"


def test_semantically_similar_returns_cached(monkeypatch):
    cache_set("chat", ["show my profit"], "result_json", 3600)
    monkeypatch.setattr(cache_mod, "_embed", lambda text: [0.999, 0.045, 0.0])
    assert cache_get("chat", ["what is my P&L"]) == "result_json"


def test_dissimilar_query_returns_none(monkeypatch):
    cache_set("chat", ["show my profit"], "result_json", 3600)
    monkeypatch.setattr(cache_mod, "_embed", lambda text: [0.0, 1.0, 0.0])
    assert cache_get("chat", ["something completely different"]) is None


def test_fail_open_on_embed_error(monkeypatch):
    monkeypatch.setattr(cache_mod, "_embed", lambda text: [])
    assert cache_get("chat", ["any question"]) is None
...
def test_fail_open_on_redis_error_set(monkeypatch):
    ...
    monkeypatch.setattr(cache_mod, "_redis_client", _BrokenRedis())
    cache_set("chat", ["any question"], "v", 3600)  # must not raise
...
def test_different_prefixes_do_not_collide():
    cache_set("prefix_a", ["same question"], "val_a", 3600)
    cache_set("prefix_b", ["same question"], "val_b", 300)
    assert cache_get("prefix_a", ["same question"]) == "val_a"
    assert cache_get("prefix_b", ["same question"]) == "val_b"
